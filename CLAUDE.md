...
- **macOS updater relaunch** — `AppHandle::restart()` / `tauri-plugin-process`'s `relaunch()` spawn `current_exe()` directly. For an `Accessory` menubar app this bypasses Launch Services and silently fails right after the updater swaps the .app bundle (the old process exits, no new menubar icon). Use `updater::relaunch_app` instead, which goes through `/usr/bin/open -n` on macOS — via a detached `sh -c 'sleep 1; …'` helper, because an immediate `open -n` races the exiting old instance: the new instance comes up while the old one still holds the global shortcut, the duplicate registration fails (`eventHotKeyExistsErr`), and startup used to die on it. Related: shortcut registration in `lib.rs` is deliberately non-fatal (log and continue) so a taken combo can never prevent the app from starting.
