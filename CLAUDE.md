...
Tauri config (`tauri.conf.json`) keeps the main window undecorated, `alwaysOnTop`, 440x680, hidden on launch, `skipTaskbar: true`. Capabilities (`capabilities/default.json`) grant `core:default`, `opener:default`, `dialog:default`, `notification:default`, `process:allow-restart`, `updater:default`, `autostart:default`.
...
- **Updater relaunch** — after `downloadAndInstall()`, the frontend calls `@tauri-apps/plugin-process`'s `relaunch()`. The plugin uses Tauri's `request_restart()` lifecycle so resources are cleaned up before the replacement process starts. Do not restore the delayed `/bin/sh` + `/usr/bin/open -n` helper: it failed to survive the old process exit in v0.17.3. Shortcut registration remains deliberately non-fatal so an externally occupied combo cannot prevent the tray from starting. Relaunch silently failed through v0.17.5 because AMFI refuses to exec a freshly written ad-hoc binary (`Code=-423`); Developer ID signing is the fix, so a release whose macOS bundle is not properly signed will reintroduce it. The relaunch path itself emits no diagnostics — reach for the macOS unified log (`/usr/bin/log show`, filtered on `amfid`/`launchd`/`syspolicyd`) rather than `auth-diagnostics.log`, and call `log` by full path since zsh shadows it with a builtin.
- **Cross-platform parity** — Code branches exist for Windows and Linux (token path, tray icon variant, tray label vs. tooltip, popup sizing), and CI builds all four targets. macOS is the primary test surface — non-mac paths are likely to have rough edges (e.g., Linux tray availability depends on the DE; GNOME needs the AppIndicator extension). Treat non-mac bugs as in-scope but possibly untested rather than out-of-scope.
- **Code signing** — macOS releases are signed with a Developer ID certificate and notarized by `tauri-cli` during the release build (no separate notarize action); It needs six repository secrets: `APPLE_CERTIFICATE` (base64 of a `.p12` holding the Developer ID identity and its private key), `APPLE_CERTIFICATE_PASSWORD`, `KEYCHAIN_PASSWORD` (any random value, for the throwaway keychain CI builds), plus `APPLE_ID`, `APPLE_PASSWORD` (an app-specific password, not the account password), and `APPLE_TEAM_ID` for notarization. Never pin `bundle.macOS.signingIdentity` in `tauri.conf.json` — `tauri-cli` derives the identity from the imported certificate and hard-errors when a pinned value disagrees. Both signing and notarization degrade *silently* to ad-hoc when their secrets are absent, which is why the release workflow asserts on the built artifact (`codesign` authority, hardened runtime, stapled ticket, `spctl`) instead of trusting the inputs. Windows and Linux are still unsigned — Windows users hit SmartScreen on first launch.
- **latest.json is assembled once, not per matrix job** — the four build jobs used to each download-merge-reupload the updater manifest, and those unsynchronised read-modify-write cycles dropped v0.17.5's Windows entries while every job still reported success. The `updater-manifest` job in `release.yml` now builds it after all uploads finish and fails if any platform's artifact or `.sig` is missing. Keep `uploadUpdaterJson: false` on the matrix jobs.
