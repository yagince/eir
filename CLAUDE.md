...
- **Code signing** — macOS releases are signed with a Developer ID certificate and notarized by `tauri-cli` during the release build (no separate notarize action); `docs/MACOS_SIGNING.md` is the runbook for the certificate, the app-specific password, and the six `APPLE_*` / `KEYCHAIN_PASSWORD` secrets. Never pin `bundle.macOS.signingIdentity` in `tauri.conf.json` — `tauri-cli` derives the identity from the imported certificate and hard-errors when a pinned value disagrees. Both signing and notarization degrade *silently* to ad-hoc when their secrets are absent, which is why the release workflow asserts on the built artifact (`codesign` authority, hardened runtime, stapled ticket, `spctl`) instead of trusting the inputs. Windows and Linux are still unsigned — Windows users hit SmartScreen on first launch.
